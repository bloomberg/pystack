...
raise_python_exception(
        ...
        std::optional<std::filesystem::path> corefile = std::nullopt)
{
    nb::object exc_type = nb::module_::import_("pystack.errors").attr(exception_name);
    nb::dict kwargs;
    if (pid) {
        kwargs["pid"] = *pid;
    }
    if (corefile) {
        kwargs["corefile"] = *corefile;
    }

    nb::tuple args = nb::make_tuple(message);
    PyObject* exc = PyObject_Call(exc_type.ptr(), args.ptr(), kwargs.ptr());
    if (!exc) {
        throw nb::python_error();
    }

    PyErr_SetObject(exc_type.ptr(), exc);
    Py_DECREF(exc);
    throw nb::python_error();
}
...
class CoreFileAnalyzerWrapper
{
  public:
    ...
    nb::set missing_modules() const
    {
        std::unordered_set<std::string> result_paths;
        std::unordered_set<std::string> missing_module_names;
        for (const auto& mod : d_extractor->missingModules()) {
            if (!isIgnoredLib(mod)) {
                result_paths.insert(mod);
                missing_module_names.insert(std::filesystem::path(mod).filename().string());
            }
        }
        for (const auto& memmap : d_extractor->MemoryMaps()) {
            ...
            if (missing_module_names.find(fname) == missing_module_names.end()) {
                result_paths.insert(path);
            }
        }

        nb::set result;
        for (const auto& path : result_paths) {
            result.add(nb::cast(std::filesystem::path(path)));
        }
        return result;
    }
...
sort_threads_by_stack_anchor(std::vector<pystack::PyThreadData> data)
{
    ...
    //  3. Index in the vector as a tie breaker (handled by stable_sort)
    std::stable_sort(
            data.begin(),
            data.end(),
...
_slice_native_stack(std::vector<pystack::PyThreadData> data)
{
    ...
    for (std::size_t i = 0; i < native_frames.size(); ++i) {
        if (pystack::is_eval_frame(native_frames[i].symbol, python_version)) {
            eval_index.push_back(i);
        }
    }

    const auto total_entry_frames = std::accumulate(
            data.begin(),
            data.end(),
            size_t{0},
    ...
    if (eval_index.size() != total_entry_frames) {
        pystack::LOG(pystack::DEBUG) << "Skipping same-tid native slicing for tid " << data[0].tid
                                     << " due to mismatched counts: entry=" << total_entry_frames
                                     << ", eval=" << eval_index.size();
        return data;
    }
    ...
    for (auto& thread_data : data) {
        ...
        const std::size_t next_thread_cursor = cursor + num_entry_frames_for_thread;
        const std::size_t next_thread_first_eval = next_thread_cursor < eval_index.size()
                                                           ? eval_index[next_thread_cursor]
                                                           : native_frames.size();
        const auto next_thread_native_frames_cursor = native_frames.begin() + next_thread_first_eval;

        thread_data.native_frames.assign(native_frames_cursor, next_thread_native_frames_cursor);
        native_frames_cursor = next_thread_native_frames_cursor;
        cursor = next_thread_cursor;
    }
    return data;
}
...
_normalize_threads(std::vector<pystack::PyThreadData> threads, NativeReportingMode native_mode)
{
    ...
    std::vector<std::vector<pystack::PyThreadData>> groups;

    for (auto& thread : threads) {
        auto [it, inserted] = tid_to_group.try_emplace(thread.tid, groups.size());
        if (inserted) {
            groups.emplace_back();
        }
        groups[it->second].push_back(std::move(thread));
    }

    std::vector<pystack::PyThreadData> ret;
    ...
    m.def(
            ...
            [](pid_t pid,
               ...
               nb::object method_obj) {
                ...
                StackMethod method;
                try {
                    method = nb::cast<StackMethod>(method_obj);
                ...
                try {
                    ...
                    raise_python_exception("EngineError", e.what(), pid);
                }
            },
            "pid"_a,
            "stop_process"_a = true,
            ...
            [](const std::filesystem::path& core_file,
               ...
               nb::object method_obj) {
                ...
                try {
                    ...
                    raise_python_exception("EngineError", e.what(), std::nullopt, core_file);
                }
            },
            "core_file"_a,
            "executable"_a,
